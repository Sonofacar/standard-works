...
- There is no test target. `make` prints target help; `make venv` runs `distclean`, recreates `.venv`, and installs the package editable into it (this is the only target that sets up the venv). `make install` copies just the package's own files (script, python library, and `scriptures.sql`) into a base directory — `PREFIX` (default `/usr/local`), staged under `DESTDIR` when set for packaging (e.g. `make install PREFIX=~/.local`, `make install PREFIX=/usr DESTDIR=/tmp/stage`); it does not touch venvs or resolve dependencies. `make check` syntax-checks the package and shim; `make run ARGS="..."` runs the CLI in place; `make clean` removes build artifacts (keeps `.venv`); `make distclean` also removes `.venv`. Nothing gates changes; verify manually.
