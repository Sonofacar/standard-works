...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data dc.tsv | awk -v cmd=list "$(get_data kjv.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data dc.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data kjv.awk)" | ${PAGER}
	done
	exit 0
fi

get_data dc.tsv | awk -v cmd=ref -v ref="$*" "$(get_data kjv.awk)" | ${PAGER}
